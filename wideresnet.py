...
class dual_t_s_WideResNet(MetaModule):
    def __init__(self, depth=28, widen_factor=2, n_classes=10, dropRate=0.0, transform_fn=None):
        super(dual_t_s_WideResNet, self).__init__()
        nChannels = [16, 16*widen_factor, 32*widen_factor, 64*widen_factor]
        assert((depth - 4) % 6 == 0)
        n = (depth - 4) / 6
        block = MetaBasicBlock
    ...
    def forward(self, x):
        ...
        out = out.view(-1, self.nChannels)
        # return self.fc(out)
        return self.fc(out), self.fc_K_plus_one_classes(out)
...
class dual_t_s_WideResNet_K_plus_one(MetaModule):
    def __init__(self, depth=28, widen_factor=2, n_classes=10, dropRate=0.0, transform_fn=None):
        super(dual_t_s_WideResNet_K_plus_one, self).__init__()
        nChannels = [16, 16*widen_factor, 32*widen_factor, 64*widen_factor]
        assert((depth - 4) % 6 == 0)
        n = (depth - 4) / 6
        block = MetaBasicBlock
        ...
        # self.fc_K_classes = MetaLinear(nChannels[3], n_classes)
        self.fc_K_plus_one_classes = MetaLinear(nChannels[3], n_classes+1)
        self.nChannels = nChannels[3]
    ...
    def forward(self, x):
        ...
        out = out.view(-1, self.nChannels)
        # return self.fc(out)
        return self.fc_K_plus_one_classes(out)
